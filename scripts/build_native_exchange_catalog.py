...
def _patches(source_root: Path) -> list[dict[str, str]]:
    ...
    if etsformer_exp.exists() and etsformer_checkpoint in etsformer_exp.read_text(encoding="utf-8"):
        patches.append(
            {
    ...
    if "fourCroguidedm2TanhR" in film_text and film_loop_anchor in film_text:
        patches.append(
            {
                "path": "run.py",
                "classification": "semantic_noop",
    ...
    fedformer_loop_replacement = """    if args.is_training:
        ...
        for ii in range(start_iteration, args.itr):
            # setting record of experiments"""
    if (
        "args.task_id" in film_text
        and "mode_select" in film_text
        and fedformer_loop_anchor in film_text
    ):
        patches.append(
            {
                "path": "run.py",

...
from finance_forecast_agent.focused_adaptive import EvidenceNode, adaptive_deterministic_advice, choose_adaptive_action, validate_evidence_refs
...
from finance_forecast_agent.focused_research import FocusedResearchController, ResearchBudget, compile_hypotheses, run_baselines
...
def _write_chart(path: Path, n: int = 1200) -> None:
    ...
    business = pd.DatetimeIndex([pd.Timestamp(session).tz_localize("America/New_York") + pd.Timedelta(hours=9, minutes=30) for session in sessions])
    returns = rng.normal(0.0002, 0.01, n)
    prices = 250 * np.cumprod(1 + returns)
    payload = {"chart": {"result": [{"meta": {"symbol": "SPY", "exchangeName": "NYSE Arca"}, "timestamp": [int(ts.tz_convert("UTC").timestamp()) for ts in business], "indicators": {"quote": [{"close": prices.tolist(), "volume": [75_000_000 + i for i in range(n)]}], "adjclose": [{"adjclose": prices.tolist()}]}}], "error": None}}
    path.write_text(json.dumps(payload), encoding="utf-8")


def _feedback(relative: float, deltas: list[float]) -> dict:
    return {"feedback_id": "fb-1", "candidate_id": "candidate-1", "metrics": {"mae": 0.01}, "relative_to_parent": {"relative_improvement": relative}, "fold_deltas_vs_parent": [{"fold_id": i, "mae_delta": value} for i, value in enumerate(deltas)]}
...
def test_compile_hypothesis_keeps_paper_fact_separate_and_requires_visible_ref() -> None:
    advice = {"hypotheses": [{"action_type": "diagnose", "statement": "Test local volatility transfer.", "mechanism": "Local hypothesis, not a rewrite of the paper claim.", "parent_candidate_id": "baseline_ridge", "model_family": "ridge_regression", "model_params": {"alpha": 2.0}, "feature_groups": ["base_lags", "volatility"], "expected_effect": "lower MAE", "counter_evidence_test": "no lower MAE", "evidence_refs": ["paper-1"]}]}
...
def test_adaptive_advice_cites_real_feedback_and_reviewed_evidence() -> None:
    prompt = {"baseline_results": [{"candidate_id": "baseline_ridge", "metrics": {"mae": 0.01}}], "prior_research_results": [], "structured_feedback": [_feedback(-0.02, [0.1, 0.1, 0.1, 0.1])], "reviewed_evidence": [EvidenceNode("paper-1", "paper_claim", "reviewed").to_dict()]}
...
def test_controller_round_two_is_feedback_driven(tmp_path: Path) -> None:
    ...
    result = FocusedResearchController(project_dir=tmp_path / "project", task=FocusedTaskSpec(), dataset=snapshot, frame=frame, budget=ResearchBudget(max_rounds=2, max_new_candidates_per_round=1, max_fit_calls=24), reviewed_evidence=[EvidenceNode("paper-1", "paper_claim", "reviewed").to_dict()]).run()
...
def test_value_benchmark_arms_share_same_budget_and_evaluator_contract(tmp_path: Path) -> None:
    ...
    arms = [run_arm(frame, arm=arm, count=2, seed=17, best_baseline_mae=best.metrics["mae"], split_spec=split_spec) for arm in ("random", "tpe_like", "one_shot_llm", "adaptive_agent")]

...
原 R0–R6 已有实现和多项真实运行证据，不重复建设。下一轮顺序：

| 批次 | 批准范围 |
|---|---|
| B0 | 产品决策、文献职责、验收条款与文档收敛 |
| B1 | 提供者可靠性、成本、调用时限、可解释暂停/恢复 |
| B2 | 目标/已有起点双入口、真实约束、正确模型交付 |
| B3 | 文献到实际实验闭环、分批自适应、有效策略/文献对照 |
| B4 | 总结、显式续接、确认预检、有限清理 |
| B5 | 逐级对抗/真实环境/陪跑验收与交付 |

从目标开始不要求已有模型或论文，但必须有合法可用数据；不支持任意市场、标签或代码。论文为两入口共享的研究依据，不是临时默认模型选择器。先做本地少量版本审核资料，不扩爬虫或向量平台。

每批先测再提交，再进入下一批。真实客户、长期服务可靠性、研究/文献增量、金融证据分别验收，不要求 Agent 必胜。内部前瞻需另行批准时点合同，完整 V3 尚未进入本轮。未来按真人需求只扩训练窗口/权重等一个维度。

以下均是历史批准快照，不与当前状态竞争；旧失败和负确认完整保留。

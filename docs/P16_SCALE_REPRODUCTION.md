...
当前是 first-batch implementation，不是 P1.8 validated。未达到 10 篇 strict 和逐篇 exploratory 执行前，不应提交为完成版或将路线图标为验收通过。
...
- strict verified：1 篇/1 claim（DLinear Exchange-Rate）。
...
下一批应优先完成 3-5 个可合法、可在当前硬件执行的 strict paper，而不是继续只扩元数据数量。每个新增 strict paper 必须有独立 MethodCard、SourceBundle、数据 hash、原生 runner、论文结果定位和容差测试。达到 10 篇后，再把逐篇 exploratory 的 28 个候选清零并处理 58 个 blocker 中高频的 adapter 缺口。

...
本轮新增的三篇论文均已有主资料校核卡。由于 live LLM provider/key 当前返回 401，它们不是实时 LLM 产物，来源在卡内明确标记。
...
原生复现展示论文值、本地值、数据与协议一致性、结果容差和治理门禁。统一基准展示共享任务 fingerprint、排行榜、每种方法的预测数量和历史 Memory 均值。
...
当前实测两种方法方向准确率均为 `0.484375`，因此不能声称有方向预测优势；LSTM 的 MAE/RMSE 略低。这一结果证明比较流程可运行，不证明策略可交易。

...
4. 点击“运行所选官方协议”。系统会在短路径运行副本中应用已登记的语义不变兼容补丁，官方 archive 和 entrypoint 本身不会被修改。
...
python scripts/run_native_claim.py --all --skip-passing
```
...
```
5. 结果审计应显示数据 hash、协议和结果容差全部通过。

当前已验证结果为 MSE `0.0810795`、MAE `0.2060906`。这是一篇论文的一个具体 claim，不应计为多篇 strict。
